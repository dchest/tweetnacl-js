...
let u64 = function(h, l) { this.hi = h|0 >>> 0; this.lo = l|0 >>> 0; }
export let gf = function(init) {
...
export function L32(x, c) { return (x << c) | (x >>> (32 - c)); }

export function ld32(x, i) {
    let u = x[i + 3] & 0xff;
    u = (u<<8)|(x[i+2] & 0xff);
    u = (u<<8)|(x[i+1] & 0xff);
...
export let crypto_box_afternm = crypto_secretbox;
